...
@Component({
  selector: 'app-lista-antiguos',
  templateUrl: './lista-antiguos.component.html',
  styleUrls: ['./lista-antiguos.component.css']
})
export class ListaAntiguosComponent implements OnInit {
  ...
  Consultar() {
    ...
    axios.post(this.ruta+'api/ConsultarApi',{
      consultasql:"SELECT @No := @No + 1 as No, CONCAT(Ap_Paterno, ' ', Ap_Materno, ' ', Nombre) Nombre_Completo , CI, Especialidad, Observacion  FROM estudiantes , (SELECT @No := 0) r WHERE Categoria='ANTIGUO' order by Ap_Paterno, Ap_Materno, Nombre"
    })
    .then(res => {

...
export class ParalelosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.consultarlog();
    this.consultarlogAdmin();
    this.cursoTittle = sessionStorage.getItem('CursoParalelo');
    this.CargarAdministrativo();
    this.CargarListadoEstudiantes(this.cursoTittle);
  }
  //#region SECCION DE COPIAS
  copiadoNombres = false;
  AccionCopiarNombres(){
    console.log('VALOR EST',JSON.parse( JSON.stringify(this.est[1])));
    // this.copyText(JSON.parse( JSON.stringify(this.est[0].Ap_Paterno)));
    var res = '';
    this.est.forEach(e => {
      var NomCompleto = e.Ap_Paterno +' '+ e.Ap_Materno+' ' + e.Nombre;
      res = res + NomCompleto+'\n';
    });
    this.copyText(res);
  ...
  AccionCopiarTodos(){
    ...
    this.est.forEach(e => {
      ...
      var Especialidad = e.Especialidad;
      var Sexo= e.Sexo;
      var NomC = e.Ap_Paterno+' '+e.Ap_Materno+' '+e.Nombre;
      res = res + NomC+'\t'+Carnet+'\t'+Sexo+'\t'+Especialidad+'\n';
      // res = res + Carnet+'\n';
    });
    this.copyText(res);
  ...
  CargarListadoEstudiantes(CursoListar) {
    ...
    axios.post(this.ruta + 'api/ListaEstudiantes', {
        ...
        NivelCurso: CursoListar
      })
      .then(res => {

        console.log('LISTA EST CARGADO: ',res.data);
        ...
        res.data.forEach(element => {
          var ApP = element.Ap_Paterno.toString()+' ';
          element.Nombre_Completo=ApP.concat(element.Ap_Materno.toString(),' ',element.Nombre.toString())

        });
        //CODIGO PARA ORDENAR NOMBRES https://www.youtube.com/watch?v=rfewUSHLymw
        res.data.sort((a,b)=>{
          if(a.Ap_Paterno>b.Ap_Paterno){
            return 1;
          }
          if(a.Ap_Paterno<b.Ap_Paterno){
            return -1;
          }
          if(a.Ap_Materno>b.Ap_Materno){
            return 1;
          }
          if(a.Ap_Paterno<b.Ap_Paterno){
            return -1;
  ...
  CargarCursosEst(estID){
    ...
    .then(res => {
      ...
      this.califest = res.data;
      this.openBoletaCalificaciones();
    }).catch(err =>  {
    console.log("err");
    });
  ...
  openBoletaCalificaciones()
  {
    ...
    var rows = this.califest; //en Esta variable estan los datos

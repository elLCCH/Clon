import axios, { AxiosResponse } from 'axios';
import { Component, ElementRef, OnInit, ViewChild } from '@angular/core';
import { Data, Router } from '@angular/router';
import { FormControl, FormGroup, Validators } from '@angular/forms';

import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { read } from 'fs';
import { EventosService } from 'src/app/Gestiones/eventos/eventos.service';
import Swal from 'sweetalert2';
import { InsEventosService } from 'src/app/Gestiones/eventos/ins-eventos.service';
pdfMake.vfs = pdfFonts.pdfMake.vfs;

@Component({
  selector: 'app-perfil',
  templateUrl: './perfil.component.html',
  styleUrls: ['./perfil.component.css']
})
export class PerfilComponent implements OnInit {


  ruta = 'http://localhost:8000/';
  //FILTRADORES O BUSCADORES
  filterP= '';
  filterT= '';
  filterV= '';
  // sesion = [];
  cursos = []; //USADO PARA CARGAR CURSOS DE LOS ESTUDIANTES... SOLO ESTUDIANTES
  MateriasTeo=[]; //MATERIAS TEORICAS... SOLO DOCENTES Y ADMINS
  MateriasPr=[]; //MATERIAS PRACTICAS... SOLO DOCENTES Y ADMINS

  sesion = {
    id: '',
    Foto: '',
    Ap_Paterno: '',
    Ap_Materno: '',
    Nombre: '',
    Sexo: '',
    FechNac: '',
    Edad: '',
    CI: '',
    Nombre_Padre: '',
    OcupacionP: '',
    NumCelP:'',
    Nombre_Madre: '',
    OcupacionM: '',
    NumCelM: '',
    Direccion: '',
    Telefono: '',
    Celular: '',
    NColegio: '',
    TipoColegio: '',
    CGrado: '',
    CNivel: '',
    Especialidad: '',
    Correo: '' ,
    Password: '',
    Estado: '',
    Matricula: '',
    Carrera: '',
    Categoria: '',
    Turno: '',
    Certificado: '',
    DocColUni: '',
    CIDoc: '',
    Boleta: '',
    Curso_Solicitado:'',
    Correo_Institucional:'',
    Admin_id:'',
    Tipo:''
  };
  NombreCompleto;
  //PARA VIDEOS
  videos = [ ];
  esAdmin;
  @ViewChild('CerrarBoton') cerrarBtn;
  @ViewChild('CerrarBotonMod') cerrarBtnMod;
  @ViewChild('CerrarModalPassword') CerrarModalPassword;
  @ViewChild('CerrarModalFoto') CerrarModalFoto;
  PublicacionSeleccionado = {
    Titulo:'',
    Descripcion:'',
    Fecha:'',
    File:'',
    Tipo:'',
    Enlace:'',
    Admin_id:''
  };


  newPublicacion = new FormGroup({
    Titulo:new FormControl(''),
    Descripcion:new FormControl(''),
    Fecha:new FormControl(''),
    File:new FormControl(''),
    Tipo:new FormControl(''),
    Enlace:new FormControl(''),
    Admin_id:new FormControl(''),
  });

  constructor(private router: Router, protected _eventos:EventosService,protected _insEvento:InsEventosService) { }

  ngOnInit(): void {
    if (localStorage.getItem('sesion')!=null) {
      console.log('SESION ACTIVA')
      let rootVar = window['rutacion'];
      this.ruta = rootVar;
      this.ColocarDatosEnVariables();
      this.consultarlog();
      console.log("DATO DE SESION",this.sesion);
      this.DiferenciadorUser();
      this.CargarVideos();
      this.CargarEventosDisponibles()
    }
    else{
      this.router.navigate(['/']);
    }

  }
//#region PARA LOS CURSOS EVENTOS
evento=[]
Spinner=false
CargarEventosDisponibles(){
  this._eventos.ObtenerEventosActivos()
  .then(res=>{
    // this.evento=res.data //ACA ESTAN LOS DISPONIBLES ACTIVOS
    res.data.forEach(e => {
      e.Estado_Pago='NO'
    });

    //FILTRAR DEPENDIENDO

      //SOLO LOS DEL INSTITUTO
      // this.evento = res.data.filter(a => (a.Acceso.indexOf('INSTITUCION')) > -1)
    // if (this.SuperAdmin=false && this.esEst==true) {
    // }
    if(localStorage.getItem('sesion') != null && (localStorage.l == 'службаданныхellcch')) {
      //ES DOCENTE
      this.evento = res.data.filter(a => (a.Acceso.indexOf('DOCENTES')) > -1 || (a.Acceso.indexOf('INSTITUCION')) > -1)
    }else{
      //ES ESTUDIANTE
      this.evento = res.data.filter(a => (a.Acceso.indexOf('ESTUDIANTES')) > -1 || (a.Acceso.indexOf('INSTITUCION')) > -1)
    }

    console.log('EVENTOS DISPONIBLES',this.evento)
  })
}

BoletaIF='invisible';

MensajeMid(){
  Swal.fire({
    title: 'SE REALIZO LA INSCRIPCIÓN CORRECTAMENTE',
    color: '#ffffff',
    showDenyButton: false,
    showCancelButton: false,
    cancelButtonColor: "#DD6B55",
    cancelButtonText:'OK',
    confirmButtonText: 'VERIFICAR INSCRIPCION',
    confirmButtonColor: "red",
    background: '#ec6e48',

  }).then((result) => {
    /* Read more about isConfirmed, isDenied below */
    if (result.isConfirmed) {
    this.router.navigate(['/ListaInscripcionesEventos']);
    }
  })
}
MostrarMensaje(iconText,tittleText){
    var Toast = Swal.mixin({
      toast: true,
      position: 'top-end',
      showConfirmButton: false,
      timer: 3000
    });
    Toast.fire({
      icon: iconText, //success, info, error,warning,question
      title: tittleText
    })
  }
newInsEvento=new FormGroup({
  Boleta_Pago:new FormControl('')
})
onFileChangeFotoEvento(event){
    if(event.target.files.length>0)
    {
      const file = event.target.files[0];
      this.newInsEvento.patchValue({
        Boleta_Pago: file
      })
      console.log(File)
    }
  }
@ViewChild('FOTO_IMG_ADD') FOTO_IMG_ADD: ElementRef;
VerificarFormatoFotoEvento(obj){
    var uploadFile = obj.files[0];
    if (!window.FileReader) {
        alert('El navegador no soporta la lectura de archivos');
        return;
    }

    if (!(/\.(jpg|png|jpeg|pdf)$/i).test(uploadFile.name)) {
        //CUANDO NO RESPETA EL FORMATO REQUERIDO
        this.MostrarMensaje('error','FORMATO INVALIDO!.');
        // alert('FORMATO INVALIDO!.');
        this.FOTO_IMG_ADD.nativeElement.value = "";
    }
    else {
      if (uploadFile.size > 1500000)
      {
        this.MostrarMensaje('warning','¡TAMAÑO EXCEDIDO!. EL TAMAÑO DEL ARCHIVO NO PUEDE EXCEDER LOS 1.5MB');
        // alert('¡TAMAÑO EXCEDIDO!. EL TAMAÑO DEL ARCHIVO NO PUEDE EXCEDER LOS 1.5MB');
        this.FOTO_IMG_ADD.nativeElement.value = "";
      }
      else {
        //EL ARCHIVO CUMPLE CON TODOS LOS REQUISITOS
      }
    }
  }

  insEvento=[];
  isSubmitted = false;
  resetInsEvento(){
    // this.newInsEvento.reset();
    this.isSubmitted =false;
    this.FOTO_IMG_ADD.nativeElement.value = "";
    this.BoletaIF='invisible'
    // this.newInsEvento.patchValue({Observacion:'SIN VERIFICAR', Estado_Pago:'NO'})
  }

  AgregarModificarInsEvento(a){

    //USAR ESTE PARA HACER CON IMAGENES
    // const formData = new FormData();
    // const nuevoInsEvento = this.newInsEvento.value;
    // Object.keys(nuevoInsEvento).forEach((key) => {
    //   formData.append(key, nuevoInsEvento[key]);
    // });


    const formData = new FormData();
    formData.append('Ap_Paterno',this.sesion.Ap_Paterno);
    formData.append('Ap_Materno',this.sesion.Ap_Materno);
    formData.append('Nombre',this.sesion.Nombre);
    formData.append('CI',this.sesion.CI);
    formData.append('Celular',(this.sesion.Celular==undefined)?'':this.sesion.Celular);
    formData.append('Institucion','IFA MARIA LUISA LUZIO');
    formData.append('Area','');
    formData.append('Estado_Pago',a.Estado_Pago);
    formData.append('Boleta_Pago',this.newInsEvento.value.Boleta_Pago);
    formData.append('Observacion','SIN VERIFICAR');
    formData.append('id_evento',a.id);
    this.isSubmitted=true;
      console.log(this.newInsEvento.value)
      this._insEvento.AgregarInsEvento(formData)
      .then(res=>{
        console.log('SE AÑADIO CORRECTAMENTE InsEvento',res.data);
        // this.resetInsEvento();
        this.Spinner=false;
        this.MensajeMid();
      })
      .catch(error=>{
        this.MostrarMensaje('error','OCURRIO UN ERROR AL INTENTAR INSCRIBIRSE')
        this.Spinner=false;
        console.log('ERROR AL AÑADIR InsEvento');
        console.log(error.response.data.message);
      })
  }
//#endregion

  SuperAdmin;
  esEst=false;
  DiferenciadorUser(){
    // DIFERENCIADOR DE EST DOC ADMIN
    if (localStorage.getItem('sesion') != null && (localStorage.l == 'службаданныхlcch')) {
      //ES ADMINISTRATIVO
      //CARGAR TODAS LAS MATERIAS (CargrTodosCursos), (CargarCursosPracticos),
      console.log('ES ADMINISTRATIVO');
      this.CargarTodosCursos();
      this.CargarCursosPracticos();
      this.SuperAdmin = true;
    }
    else
    {
      if (localStorage.getItem('sesion') != null && (localStorage.l == 'службаданныхellcch')) {
        //ES DOCENTE
        //CARGAR SUS MATERIAS (CARGARTODOSCURSOS) Y TAMBIEN CARGAR TODOS LOS CURSOS PRACTICOS
        this.CargarMisCursos(this.sesion.id);
        this.CargarMisCursosPracticos(this.sesion.id);
        console.log('ES DOCENTE');
        this.SuperAdmin = false;
      }
      else
      {
        //ES ESTUDIANTE
        //CARGAR SOLO SUS MATERIAS(CARGARCUSRSOSEST), LAS DEMAS PESTAÑAS OCULTAR
        console.log('ES ESTUDIANTE');

        this.CargarCursosEst(this.sesion.id); //ESTO ES ESPECIAL SOLO ESTUDIANTES
        this.SuperAdmin = false;
        this.esEst = true;
      }
    }
  }
  cursoData=[];
  GestionarCalificaciones(){
    // DIFERENCIADOR DE EST DOC ADMIN
    if (localStorage.getItem('sesion') != null && (localStorage.l == 'службаданныхlcch')) {
      //ES ADMINISTRATIVO
      //CARGAR TODAS LAS MATERIAS (CargrTodosCursos), (CargarCursosPracticos),
      console.log('ES ADMINISTRATIVO, PUEDE ACCEDER A TODO');
      localStorage.setItem('cursoData', JSON.stringify(this.cursoData));

      this.router.navigate(['/calificaciones']);
    }
    else
    {
      if (localStorage.getItem('sesion') != null && (localStorage.l == 'службаданныхellcch')) {
        //ES DOCENTE
        //CARGAR SUS MATERIAS (CARGARTODOSCURSOS) Y TAMBIEN CARGAR TODOS LOS CURSOS PRACTICOS
        console.log('ES DOCENTE, ENTRANDO A SU CURSO');
        localStorage.setItem('cursoData', JSON.stringify(this.cursoData));
        this.router.navigate(['/calificaciones']);
      }
      else
      {
        //ES ESTUDIANTE
        //CARGAR SOLO SUS MATERIAS(CARGARCUSRSOSEST), LAS DEMAS PESTAÑAS OCULTAR
        console.log('ES ESTUDIANTE NO PUEDE INGRESAR');
        this.router.navigate(['/Inicio']);
      }
    }
  }

  CargarEstudiante()
  {
    // axios.post(this.ruta+'api/ConsultarApi',{
    //   consultasql:'select Ap_Paterno,Ap_Materno,Nombre, CI from estudiantes '
    // })
    // .then(res => {
    //   res.data.forEach(element => {
    //     if (element.Ap_Paterno != '') {
    //       //EXISTE
    //       console.log('TRUE');
    //       element.Visible = true;
    //     } else {
    //       //NO EXISTE
    //       console.log('FALSE');
    //       element.Visible = false;
    //     }
    //   });
    //   console.log('CURSOS PRACTICOS SON: ',res.data);
    //   this.MateriasPr= res.data;
    // }).catch(err =>  {
    // console.log("err");
    // });
  }

  CargarCursosPracticos()
  {
    axios.post(this.ruta+'api/ConsultarApi',{
      consultasql:'select * from cursos where NombreCurso LIKE "%INSTRUMENTO DE ESPECIALIDAD%" order by Sigla'
      // consultasql:'SELECT DISTINCT cursos.NivelCurso as Distinto, cursos.* , administrativos.Ap_Paterno, administrativos.Ap_Materno, administrativos.Nombre FROM estudiantes LEFT JOIN administrativos ON estudiantes.Admin_id = administrativos.id LEFT JOIN calificaciones ON calificaciones.estudiante_id = estudiantes.id LEFT JOIN cursos ON calificaciones.curso_id = cursos.id WHERE NombreCurso LIKE "%INSTRUMENTO DE ESPECIALIDAD%"'
    })
    .then(res => {
      res.data.forEach(element => {
        if (element.Ap_Paterno != '') {
          //EXISTE
          console.log('TRUE');
          element.Visible = true;
        } else {
          //NO EXISTE
          console.log('FALSE');
          element.Visible = false;
        }
      });
      console.log('CURSOS PRACTICOS SON: ',res.data);
      this.MateriasPr= res.data;
    }).catch(err =>  {
    console.log("err");
    });

  }
  CargarMisCursosPracticos(adminID)
  {
    axios.post(this.ruta+'api/ConsultarApi',{
      consultasql:'SELECT DISTINCT cursos.NivelCurso as Distinto, cursos.* , administrativos.Ap_Paterno, administrativos.Ap_Materno, administrativos.Nombre FROM estudiantes LEFT JOIN administrativos ON estudiantes.Admin_id = administrativos.id LEFT JOIN calificaciones ON calificaciones.estudiante_id = estudiantes.id LEFT JOIN cursos ON calificaciones.curso_id = cursos.id WHERE NombreCurso LIKE "%INSTRUMENTO DE ESPECIALIDAD%" AND administrativos.id = '+adminID+''
    })
    .then(res => {
      res.data.forEach(element => {
        if (element.Ap_Paterno != '') {
          //EXISTE
          console.log('TRUE');
          element.Visible = true;
        } else {
          //NO EXISTE
          console.log('FALSE');
          element.Visible = false;
        }
      });
      console.log('CURSOS PRACTICOS SON: ',res.data);
      this.MateriasPr= res.data;
    }).catch(err =>  {
    console.log("err");
    });

  }
  CargarCursosEst(estID){
    //USADO PARA CARGAR LOS CURSOS DE UN ESTUDIANTE
    axios.post(this.ruta+'api/ConsultarApi',{
      // consultasql:'SELECT calificaciones.Primero,calificaciones.Segundo,calificaciones.Tercero,calificaciones.Cuarto,calificaciones.Promedio,calificaciones.curso_id, calificaciones.estudiante_id, estudiantes.Ap_Paterno,estudiantes.Ap_Materno,estudiantes.Nombre,cursos.NombreCurso, cursos.NivelCurso,cursos.Tipo FROM calificaciones LEFT JOIN estudiantes ON calificaciones.estudiante_id = estudiantes.id LEFT JOIN cursos ON calificaciones.curso_id = cursos.id where calificaciones.estudiante_id=793'
      //ESTA CONSULTA ES FINISH
      consultasql:'SELECT administrativos.id as Admin_id,administrativos.Foto,administrativos.Ap_Paterno,administrativos.Ap_Materno,administrativos.Nombre,calificaciones.PruebaRecuperacion,calificaciones.Primero,calificaciones.Segundo,calificaciones.Tercero,calificaciones.Cuarto,calificaciones.PromEvT,calificaciones.PromEvP,calificaciones.Primero,calificaciones.Segundo,calificaciones.Tercero,calificaciones.Cuarto,calificaciones.Promedio,calificaciones.Teorica1,calificaciones.Teorica2,calificaciones.Teorica3,calificaciones.Teorica4, calificaciones.Practica1,calificaciones.Practica2,calificaciones.Practica3,calificaciones.Practica4,calificaciones.curso_id as Curso_id, calificaciones.estudiante_id,cursos.NombreCurso, cursos.NivelCurso,cursos.Tipo,cursos.Sigla,cursos.BiTriEstado FROM calificaciones LEFT JOIN estudiantes ON calificaciones.estudiante_id = estudiantes.id LEFT JOIN cursos ON calificaciones.curso_id = cursos.id  LEFT JOIN administrativos__cursos ON cursos.id = administrativos__cursos.Curso_id LEFT JOIN administrativos ON administrativos__cursos.Admin_id= administrativos.id where estudiante_id = '+estID+''
    })
    .then(res => {

      // var keyNames = Object.keys(res.data[0] );
      // keyNames.forEach(element => {
      //   this.Columnas.push({'titulo':element});
      //   // console.log('KEY', element);
      // });
      // console.log('KEY', this.Columnas);
      // // console.log(keyNames); // Outputs ["a","b","c"]
      res.data.forEach(element => {
        // var ApP=element.Ap_Paterno;
        // var ApPest=element.Ap_Paterno_est;
        if (element.Ap_Paterno != '') {
          //EXISTE
          console.log('TRUE');
          element.Visible = true;
        } else {
          //NO EXISTE
          console.log('FALSE');
          element.Visible = false;

        }

      });
      console.log('MIS CURSOS SON: ',res.data);
      this.cursos = res.data;
    }).catch(err =>  {
    console.log("err");
    });

  }
  CargarTodosCursos(){
    //USADO PARA CARGAR TODOS LOS CURSOS QUE TENGAN DOCENTE
    axios.post(this.ruta+'api/ConsultarApi',{
      consultasql:'SELECT administrativos__cursos.Admin_id,administrativos__cursos.Curso_id,administrativos.Ap_Paterno,administrativos.Ap_Materno,administrativos.Nombre,administrativos.Foto, cursos.NombreCurso,cursos.NivelCurso,cursos.Sigla,cursos.Tipo,cursos.BiTriEstado FROM administrativos__cursos LEFT JOIN administrativos ON administrativos__cursos.Admin_id = administrativos.id LEFT JOIN cursos ON administrativos__cursos.Curso_id = cursos.id'
    })
    .then(res => {
      res.data.forEach(element => {
        // var ApP=element.Ap_Paterno;
        // var ApPest=element.Ap_Paterno_est;
        if (element.Ap_Paterno != '') {
          //EXISTE
          console.log('TRUE');
          element.Visible = true;
        } else {
          //NO EXISTE
          console.log('FALSE');
          element.Visible = false;

        }

      });
      console.log('MIS CURSOS SON: ',res.data);
      this.MateriasTeo = res.data;
      // var keyNames = Object.keys(res.data[0] );
      // keyNames.forEach(element => {
      //   this.Columnas.push({'titulo':element});
      //   // console.log('KEY', element);
      // });
      // console.log('KEY', this.Columnas);
      // // console.log(keyNames); // Outputs ["a","b","c"]
      // res.data.forEach(element => {
      //   element.Editando=false;
      // });
    }).catch(err =>  {
    console.log("err");
    });
  }
  CargarMisCursos(adminID){
    //GENERALMENTE USADO PARA CARGAR LOS CURSOS DE UN PROFESOR
    axios.post(this.ruta+'api/ConsultarApi',{
      consultasql:'SELECT administrativos__cursos.Admin_id,administrativos__cursos.Curso_id,administrativos.Ap_Paterno,administrativos.Ap_Materno,administrativos.Nombre,administrativos.Foto, cursos.NombreCurso,cursos.NivelCurso,cursos.Sigla,cursos.Tipo,cursos.BiTriEstado FROM administrativos__cursos LEFT JOIN administrativos ON administrativos__cursos.Admin_id = administrativos.id LEFT JOIN cursos ON administrativos__cursos.Curso_id = cursos.id where Admin_id= '+adminID+''
    })
    .then(res => {
      res.data.forEach(element => {
        // var ApP=element.Ap_Paterno;
        // var ApPest=element.Ap_Paterno_est;
        if (element.Ap_Paterno != '') {
          //EXISTE
          console.log('TRUE');
          element.Visible = true;
        } else {
          //NO EXISTE
          console.log('FALSE');
          element.Visible = false;

        }

      });
      console.log('MIS CURSOS SON: ',res.data);
      this.MateriasTeo = res.data;
      // var keyNames = Object.keys(res.data[0] );
      // keyNames.forEach(element => {
      //   this.Columnas.push({'titulo':element});
      //   // console.log('KEY', element);
      // });
      // console.log('KEY', this.Columnas);
      // // console.log(keyNames); // Outputs ["a","b","c"]
      // res.data.forEach(element => {
      //   element.Editando=false;
      // });
    }).catch(err =>  {
    console.log("err");
    });
  }
  ColocarDatosEnVariables()
  {
    var obj = JSON.parse(localStorage.getItem('sesion'));
    this.sesion= (Array.of(obj)[0].data);
    // console.log(sujeto);
    // this.sesion = sujeto;
    this.NombreCompleto = this.sesion.Ap_Paterno+' '+this.sesion.Ap_Materno+' '+this.sesion.Nombre;
  }
  private consultarlog() {
    if (localStorage.getItem('sesion') != null && (localStorage.l == 'службаданныхlcch' || localStorage.l == 'службаданныхellcch')) {
      //ES DOCENTE O ADMINISTRATIVO
      this.esAdmin = false;
    }
    else
    {
      //ES ESTUDIANTE
      this.esAdmin = true;
    }
  }
  newEstudiante = new FormGroup({
    Foto: new FormControl(''),
    Password: new FormControl('')
  });
  MensajeContenido;
  @ViewChild('FotoInput') myInputFoto: ElementRef;
  //PARA FOTOS
  onFileChange(event) {
    if (event.target.files.length > 0) {
      const file = event.target.files[0];

      this.newEstudiante.patchValue({
        Foto: file
      })
      console.log(file);
    }
  }
  VerificarFormatoFoto(obj){
    console.log('VERIFICANDO FORMATO...');
    var uploadFile = obj.files[0];
    if (!window.FileReader) {
        alert('El navegador no soporta la lectura de archivos');
        return;
    }

    if (!(/\.(jpg|png|jpeg)$/i).test(uploadFile.name)) {
        // alert('El archivo a adjuntar no es una imagen');
        //CUANDO NO RESPETA EL FORMATO REQUERIDO
        if(confirm("FORMATO INVÁLIDO!! ")) {
          this.myInputFoto.nativeElement.value = "";
        }else{
          this.myInputFoto.nativeElement.value = "";
        }

    }
    else {
      if (uploadFile.size > 1500000)
      {
        // alert('El peso de la imagen no puede exceder los 200kb')
        if(confirm("¡TAMAÑO EXCEDIDO!. Por favor el peso de la Foto no puede exceder 1.5MB ")) {
          this.myInputFoto.nativeElement.value = "";
        }else{
          this.myInputFoto.nativeElement.value = "";
        }
      }
      else {
        //LA IMAGEN CUMPLE CON TODOS LOS REQUISITOS
          // alert('Imagen correcta :)')
      }
    }
  }
  CambiarFoto(){


    if (this.esEst == true) {
      //ES ESTUDIANTE
      const formData1 = new FormData();
      formData1.append('Foto', this.newEstudiante.value.Foto);
      formData1.append('Password', this.sesion.Password);
      axios({
          method: 'post',
          url: this.ruta + 'api/EstudianteUpdate/' + this.sesion.id,
          data: formData1,
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        })
        .then(res => {
          // console.log('SE MODIFICO CORRECTAMENTE');
          console.log('SE LOGRO CAMBIAR LA FOTO');
          console.log(res);
          this.CerrarModalFoto.nativeElement.click();
          if(confirm("FOTO MODIFICADA CON ÉXITO!!, para Aplicar Cambios Desea Cerrar Sesión? ")) {
            localStorage.clear();
            // this.router.navigate(['/Login']);
            document.location.href = '/Inicio';
          }
        })
        .catch(error => {
          console.log('NO SE CAMBIO LA FOTO');
          console.log(error);
        })
    } else {
      //ES DEL PLANTEL ADMINISTRATIVO O DOCENTE
      const formData1 = new FormData();
      formData1.append('Foto', this.newEstudiante.value.Foto);
      formData1.append('Password', this.sesion.Password);
      axios({
        method: 'post',
        url: this.ruta + 'api/AdministrativoUpdate/' + this.sesion.id,
        data: formData1,
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      })
      .then(res => {
        // console.log('SE MODIFICO CORRECTAMENTE');
        console.log('SE LOGRO CAMBIAR LA FOTO');
        console.log(res);
        this.CerrarModalFoto.nativeElement.click();
        if(confirm("FOTO MODIFICADA CON ÉXITO!!, para Aplicar Cambios Desea Cerrar Sesión? ")) {
          localStorage.clear();
          // this.router.navigate(['/Login']);
          document.location.href = '/Inicio';
        }
      })
      .catch(error => {
        console.log('NO SE CAMBIO LA FOTO');
        console.log(error);
      })
    }


  }
  AgregarVideo()
  {
    //para prueba
    // this.cerrarBtn.nativeElement.click();
    // return;
    //hasta aca
    const formData = new FormData();
    formData.append('Titulo',this.newPublicacion.value.Titulo);
    formData.append('Descripcion',this.newPublicacion.value.Descripcion);
    formData.append('Fecha',this.newPublicacion.value.Fecha);
    formData.append('File',this.newPublicacion.value.File);
    formData.append('Enlace',this.newPublicacion.value.Enlace);
    formData.append('Tipo','VideoPublico');
    formData.append('Admin_id',localStorage.numberADD);

    axios({
      method:'post',
      url:this.ruta+'api/Publicacion',
      data:formData,
      headers:{'Content-Type':'multipart/form-data'}
    })
    .then(res=>{
      console.log('SE AÑADIO CORRECTAMENTE');
      console.log(res);
      this.cerrarBtn.nativeElement.click();
      this.CargarVideos();
    })
    .catch(error=>{
      console.log('Error al Agregar');
      console.log(error);
    })
  }
  EliminarVideo(Video)
  {
    if(confirm("Seguro que desea Eliminar el Video? ")) {
        // const IdAdmin = 3;
    const formData = new FormData();
    formData.append('NombrePublicacion',Video.nombrePublicacion);
    formData.append('NivelPublicacion',Video.nivelPublicacion);

    axios({
      method:'delete',
      url:this.ruta+'api/Publicacion/'+Video.id
    })
    .then(res=>{
      console.log('SE ELIMINO CORRECTAMENTE');
      console.log(res);
      this.CargarVideos();
    })
    .catch(error=>{
      console.log('HAY ERROR AL BORRAR');
      console.log(error);
    })
    }
  }
  CargarVideos() {
    axios.get(this.ruta+'api/Publicacion?tipo=VideoPublico')
    .then(res => {
      console.log(res.data);
      this.videos = res.data;
      res.data.forEach(element => {
        var dateFech = new Date(element.Fecha.toString());
        let strFech = dateFech.toLocaleDateString('Es-es');
        element.Fecha = strFech;
      });

    }).catch(err =>  {
    console.log("err");
    });
  }


  contrasenia; //CONTRASEÑA DE AUTENTICACION
  IncorrectaPass=false; //MENSAJE DE CONTRASEÑA INCORRECTA

  contraseniaN1; //NUEVA CONTRASEÑA 1
  contraseniaN2; //NUEVA CONTRASEÑA 2
  IncorrectaPassN=false; //MENSAJE DE NO COINCIDEN
  autenticar() {
    const formData = new FormData();

    formData.append('CI', this.sesion.CI);
    formData.append('Password', this.contrasenia);

    axios({
        method: 'post',
        url: this.ruta + 'api/AdministrativoAUTH', //llamada a Autentificar del AdministrativosController
        data: formData,
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      })
      .then(res => {
        var x = res;

        console.log(x.data);
        if (x.data != 'NOLOG') {
          if (x.data.Estado == "ACTIVO" && x.data.Tipo == "ADMINISTRATIVO") {

            console.log('SE LOGEO CORRECTAMENTE');
            console.log(res);
            this.IncorrectaPass = false;
            if (this.contraseniaN1!=this.contraseniaN2 || this.contraseniaN1 == null || this.contraseniaN1 == '' || this.contraseniaN2 == null || this.contraseniaN2 == '') {
              this.IncorrectaPassN = true;
            } else {
              //TODO BIEN; CAMBIAMOS LA CONTRASEÑA DEL ADMINISTRATIVO
              this.IncorrectaPassN = false;
              axios.post(this.ruta + 'api/AdministrativoUpdate/' + this.sesion.id, {
                Password: this.contraseniaN1
              })
              .then(res => {
                // console.log('SE MODIFICO CORRECTAMENTE');
                console.log('SE LOGRO CAMBIAR LA CONTRASEÑA');
                console.log(res);
                this.CerrarModalPassword.nativeElement.click();
                if(confirm("CONTRASEÑA MODIFICADA CON ÉXITO!!,  Se Cerrará la sesión ")) {
                  localStorage.clear();
                  // this.router.navigate(['/Login']);
                  document.location.href = '/Inicio';
                }else{
                  localStorage.clear();
                  // this.router.navigate(['/Login']);
                  document.location.href = '/Inicio';
                }
              })
              .catch(error => {
                console.log('NO SE CAMBIO LA CONTRASEÑA');
                console.log(error);
              })
            }
            this.router.navigate(['/Perfil']);
          }
          //ESTO ES PARA LOS DOCENTES QUE QUIEREN INICIAR SESION

          if (x.data.Estado == "ACTIVO" && x.data.Tipo == "DOCENTE") {
            console.log('SE LOGEO CORRECTAMENTE');
            console.log(res);
            this.router.navigate(['/Perfil']);
            this.IncorrectaPass = false;
            if (this.contraseniaN1!=this.contraseniaN2 || this.contraseniaN1 == null || this.contraseniaN1 == '' || this.contraseniaN2 == null || this.contraseniaN2 == '') {
              this.IncorrectaPassN = true;
            } else {
              //TODO BIEN; CAMBIAMOS LA CONTRASEÑA  DEL ADMINISTRATIVO
              this.IncorrectaPassN = false;
              axios.post(this.ruta + 'api/AdministrativoUpdate/' + this.sesion.id, {
                Password: this.contraseniaN1
              })
              .then(res => {
                // console.log('SE MODIFICO CORRECTAMENTE');
                console.log('SE LOGRO CAMBIAR LA CONTRASEÑA');
                console.log(res);
                this.CerrarModalPassword.nativeElement.click();
                if(confirm("CONTRASEÑA MODIFICADA CON ÉXITO!!, Se Cerrará Sesión ")) {
                  localStorage.clear();
                  // this.router.navigate(['/Login']);
                  document.location.href = '/Inicio';
                }else{
                  localStorage.clear();
                  // this.router.navigate(['/Login']);
                  document.location.href = '/Inicio';
                }
              })
              .catch(error => {
                console.log('NO SE CAMBIO LA CONTRASEÑA');
                console.log(error);
              })
            }
          }
        } else {
          //NO ESTABA ACTIVO
          this.IncorrectaPass = true;
        }

      })
      .catch(error => {
        // console.log('ERROR AL LOGEARSE');
        // this.Auth=true;
        console.log('BUSCANDO POR ESTUDIANTES');


        //SI NO EXISTE ADMIN ENTONCES BUSCAR LISTA DE ESTUDIANTES
        axios({
            method: 'post',
            url: this.ruta + 'api/EstudianteAUTH',
            data: formData,
            headers: {
              'Content-Type': 'multipart/form-data'
            }
          })
          .then(res => {

            var x = res;

            console.log(x.data);
            if (x.data != 'NOLOG') {

              if (x.data.Estado == "ACTIVO") {
                console.log('SE LOGEO CORRECTAMENTE');
                console.log(res);
                this.IncorrectaPass = false;
                if (this.contraseniaN1!=this.contraseniaN2 || this.contraseniaN1 == null || this.contraseniaN1 == '' || this.contraseniaN2 == null || this.contraseniaN2 == '') {
                  this.IncorrectaPassN = true;
                } else {
                  //TODO BIEN; CAMBIAMOS LA CONTRASEÑA DEL ESTUDIANTE
                  this.IncorrectaPassN = false;
                  axios.post(this.ruta + 'api/EstudianteUpdate/' + this.sesion.id, {
                    Password: this.contraseniaN1
                  })
                  .then(res => {
                    // console.log('SE MODIFICO CORRECTAMENTE');
                    console.log('SE LOGRO CAMBIAR LA CONTRASEÑA');
                    console.log(res);
                    this.CerrarModalPassword.nativeElement.click();
                    if(confirm("CONTRASEÑA MODIFICADA CON ÉXITO!!, Desea Cerrar Sesión? ")) {
                      localStorage.clear();
                      // this.router.navigate(['/Login']);
                      document.location.href = '/Inicio';
                    }
                  })
                  .catch(error => {
                    console.log('NO SE CAMBIO LA CONTRASEÑA');
                    console.log(error);
                  })
                }
                this.router.navigate(['/Perfil']);
              } else {
                //EXISTE PERO NO ESTABA ACTIVO
                this.IncorrectaPass = true;
              }
            } else {
              console.log('ERROR AL LOGEARSE');
              this.IncorrectaPass = true;
            }


          })
          .catch(error => {
            console.log('ERROR AL LOGEARSE');

            this.IncorrectaPass = true;
          })
      })
  }
  writeRotatedText = function(text,text1,text2) {
    var ctx, canvas = document.createElement('canvas');
    // I am using predefined dimensions so either make this part of the arguments or change at will
    canvas.width = 90;
    canvas.height = 300;
    ctx = canvas.getContext('2d');
    ctx.font = 'bold 21pt Arial';
    if (text2!='') {
      //si tiene 3 filas
      canvas.width = 120;
      canvas.height = 290;
      ctx.font = 'bold 30pt Arial';
    }

    ctx.textAlign = "center";
    ctx.save();
    ctx.translate(36,145);
    ctx.rotate(-0.5*Math.PI);
    ctx.fillStyle = '#000';
    ctx.fillText(text , 0, 0);
    ctx.fillText(text1 , 0, 40);
    if (text2!='') {
      ctx.fillText(text , 0, 80);
    }


    ctx.restore();

    return canvas.toDataURL();
  };
  writeRotatedText2 = function(text,text1,text2) {
    var ctx, canvas = document.createElement('canvas');
    // I am using predefined dimensions so either make this part of the arguments or change at will
    canvas.width = 90;
    canvas.height = 250;
    ctx = canvas.getContext('2d');
    ctx.font = 'bold 25pt Arial';
    if (text2!='') {
      //si tiene 3 filas
      canvas.width = 120;
      canvas.height = 290;
      ctx.font = 'bold 30pt Arial';
    }

    ctx.textAlign = "center";
    ctx.save();
    ctx.translate(40,119);
    ctx.rotate(-0.5*Math.PI);
    ctx.fillStyle = '#000';
    ctx.fillText(text , 0, 0);
    ctx.fillText(text1 , 0, 40);
    if (text2!='') {
      ctx.fillText(text , 0, 80);
    }


    ctx.restore();

    return canvas.toDataURL();
  };
  openBoletaCalificaciones1()
  {
    var headers = {
      fila_0: {
        col_0: {
          text: 'Nº',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          alignment: 'center',
          margin: [0, 30, 0, 0],
          bold: true
        },
        col_1: {
          text: 'ASIGNATURAS',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2 ,
          alignment: 'center',
          margin: [0, 30, 0, 0],
          bold: true
        },
        col_2: {
          text: 'PRIMERA \n EVALUACIÓN',
          style: 'tableHeader',
          colSpan: 2,
          alignment: 'center',
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_3: {
          text: '',
        },
        col_4: {
          text: 'SEGUNDA \n EVALUACIÓN',
          style: 'tableHeader',
          colSpan: 2,
          alignment: 'center',
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_5: {
          text: '',
        },
        col_6: {
          text: 'TERCERA \n EVALUACIÓN',
          style: 'tableHeader',
          colSpan: 2,
          alignment: 'center',
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_7: {
          text: '',
        },
        col_8: {
          text: 'CUARTA \n EVALUACIÓN',
          style: 'tableHeader',
          colSpan: 2,
          alignment: 'center',
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_9: {
          text: '',
        },
        col_10: {
          // text: 'PROM. \n TEÓRICO',
          // style: 'tableHeader',
          // colSpan: 1,
          // rowSpan:2,
          // alignment: 'center',
          // margin: [0, 15, 0, 0],
          // bold: true
          image: this.writeRotatedText('PROMEDIO','TEÓRICO',''), fit:[25,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_11: {
          // text: 'PROM. \n PRÁCTICO',
          // style: 'tableHeader',
          // colSpan: 1,
          // rowSpan:2,
          // alignment: 'center',
          // margin: [0, 15, 0, 0],
          // bold: true
          image: this.writeRotatedText('PROMEDIO','PRÁCTICO',''), fit:[25,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_12: {
          // text: 'PROM.\nFINAL',
          // style: 'tableHeader',
          // colSpan: 1,
          // rowSpan:2,
          // alignment: 'center',
          // margin: [0, 15, 0, 0],
          // bold: true
          image: this.writeRotatedText('PROMEDIO','FINAL',''), fit:[25,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_13: {
          // text: 'PRUEBA DE\n RECUPERACIÓN',
          // style: 'tableHeader',
          // colSpan: 1,
          // rowSpan:2,
          // alignment: 'center',
          // margin: [0, 15, 0, 0],
          // bold: true
          image: this.writeRotatedText('PRUEBA DE','RECUPERACIÓN',''), fit:[25,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_14: {
          text: 'OBSERVACIÓN',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          alignment: 'center',
          margin: [0, 30, 0, 0],
          bold: true
        },
      },

      fila_1: {
        col_0: {
          text: '',
        },
        col_1: {
          text: '',
        },
        col_2: {
          image: this.writeRotatedText2('EVALUACIÓN','TEÓRICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_3: {
          image: this.writeRotatedText2('EVALUACIÓN','PRÁCTICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_4: {
          image: this.writeRotatedText2('EVALUACIÓN','TEÓRICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_5: {
          image: this.writeRotatedText2('EVALUACIÓN','PRÁCTICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_6: {
          image: this.writeRotatedText2('EVALUACIÓN','TEÓRICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_7: {
          image: this.writeRotatedText2('EVALUACIÓN','PRÁCTICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_8: {
          image: this.writeRotatedText2('EVALUACIÓN','TEÓRICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_9: {
          image: this.writeRotatedText2('EVALUACIÓN','PRÁCTICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_10: {
          text: '',
        },
        col_11: {
          text: '',
        },
        col_12: {
          text: '',
        },
        col_13: {
          text: '',
        },
        col_14: {
          text: '',
        },
      }
    }


    var rows = this.cursos;

    var body = [];
    for (var key in headers) {
      if (headers.hasOwnProperty(key)) {
        var header = headers[key];
        var row = new Array();
        row.push(header.col_0);
        row.push(header.col_1);
        row.push(header.col_2);
        row.push(header.col_3);
        row.push(header.col_4);
        row.push(header.col_5);
        row.push(header.col_6);
        row.push(header.col_7);
        row.push(header.col_8);
        row.push(header.col_9);
        row.push(header.col_10);
        row.push(header.col_11);
        row.push(header.col_12);
        row.push(header.col_13);
        row.push(header.col_14);

        body.push(row);
      }
    }
    //HASTA ACA TODO BIEN
    var contador = 0;
    for (var key in rows) {

      if (rows.hasOwnProperty(key)) {
        var data = rows[key];
        var row = new Array();
          contador = contador + 1;
          row.push({ text: contador, alignment: 'center',style: 'tablecontent'});
          // row.push( data.id.toString()  ); //0

          //PARA NOMBRE COMPLETO
          // var apPa = data.Ap_Paterno.toString();
          // var apMa = data.Ap_Materno.toString();
          // var nom = data.Nombre.toString();
          // var NomComplet = apPa + ' ' + apMa + ' ' + nom;

          row.push({ text: data.NombreCurso.toString(), alignment: 'center',style: 'tablecontent'}); //1
          row.push({ text: data.Teorica1.toString(), alignment: 'center',style: 'tablecontent'}); //2
          row.push({ text: data.Practica1.toString(), alignment: 'center',style: 'tablecontent'}); //3
          row.push({ text: data.Teorica2.toString(), alignment: 'center',style: 'tablecontent'}); //4
          row.push({ text: data.Practica2.toString(), alignment: 'center',style: 'tablecontent'}); //5
          row.push({ text: data.Teorica3.toString(), alignment: 'center',style: 'tablecontent'}); //6
          row.push({ text: data.Practica3.toString(), alignment: 'center',style: 'tablecontent'}); //7
          row.push({ text: data.Teorica4.toString(), alignment: 'center',style: 'tablecontent'}); //8
          row.push({ text: data.Practica4.toString(), alignment: 'center',style: 'tablecontent'}); //9
          row.push({ text: data.PromEvT.toString(), alignment: 'center',style: 'tablecontent'}); //10
          row.push({ text: data.PromEvP.toString(), alignment: 'center',style: 'tablecontent'}); //11
          row.push({ text: data.Promedio.toString(), alignment: 'center',style: 'tablecontent'}); //12
          row.push({ text: data.PruebaRecuperacion.toString(), alignment: 'center',style: 'tablecontent'}); //13

          if (data.Primero.toString()!='0') {
            row.push({ text: data.Primero.toString(), alignment: 'center',style: 'tablecontent'}); //14
          } else {
            row.push({ text: 'NINGUNA OBSERVACIÓN', alignment: 'center',style: 'tablecontent'}); //14
          }


          body.push(row);

      }
    }
    // var paginaInicial = this.numeroPaginaCurso - 1;
    var dd = {
      //izq, arriba,derecha

      pageSize: 'LETTER',
      pageMargins: [ 40, 30, 40, 60 ],
      // pageOrientation: 'landscape',

      // footer: function (currentPage, pageCount) {
      //   return [{
      //       text: '' + (currentPage + paginaInicial).toString() + '',
      //       alignment: 'right',
      //       margin: [0, -700, 50, 0],fontSize: 9
      //     },

      //   ];
      // },
      defaultStyle: {
        fontSize: 8,
        columnGap: 20
      },
      content: [
        {text:'INSTITUTO DE FORMACIÓN ARTISTICA', fontSize:9, alignment: 'center', bold:true},
        {text:'" MARÍA LUISA LUZIO "', fontSize:9, alignment: 'center', bold:true},
        {text:' BOLETÍN DE CALIFICACIONES ', fontSize:9, alignment: 'center', bold:true},
        // {text:'\n'},
        {
          image: 'data:image/png;base64,',
          width: 60, height: 60,  alignment:'right',margin:[0,-45,0,0]
        },
        {
          columns: [
            {
              width: '*',
              text: this.NombreCompleto,
              bold: true
            },
            {
              width: '*',
              text: data.NivelCurso.toString(),
              bold: true
            },
            {
              width: '*',
              text: 'GESTIÓN 2022',
              bold: true
            },
          ]
        },
        {
          style: 'tableExample',
          table: {
            widths: ['auto', 'auto','auto','auto','auto','auto','auto','auto','auto','auto','auto','auto','auto','auto','*'],
            // headerRows: 1,
            // keepWithHeaderRows: 1,
            body: body
          },

          layout: {
            hLineWidth: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 2 : 1;
            },
            vLineWidth: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 2 : 1;
            },
            hLineColor: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 'black' : 'gray';
            },
            vLineColor: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 'black' : 'gray';
            },
            // hLineStyle: function (i, node) { return {dash: { length: 10, space: 4 }}; },
            // vLineStyle: function (i, node) { return {dash: { length: 10, space: 4 }}; },
            // paddingLeft: function(i, node) { return 4; },
            // paddingRight: function(i, node) { return 4; },
            // paddingTop: function(i, node) { return 2; },
            // paddingBottom: function(i, node) { return 2; },
            // fillColor: function (rowIndex, node, columnIndex) { return null; }
          }
        },
        {
          text:'\n\n\n'
        },
        {
          columns: [
            {
              alignment:'center',
              width: '*',
              text: '_____________________________ \nDIRECTOR ACADÉMICO',
              marginLeft: 5,
              bold: true
            },
            {
              alignment:'center',
              width: '*',
              text: '___________________________ \nPADRE DE FAMILIA',
              marginLeft: 100,
              bold: true
            },
          ]
        },
      ],

      styles: {
        headers: {
          fontSize: 28,
          bold: true
        },
        subheader: {
          fontSize: 15,
          bold: true
        },
        quote: {
          italics: true
        },
        small: {
          fontSize: 8
        },
        sta: {
          fontSize: 11,
          bold: false,
          alignment: 'justify'
        },
        tableHeader: {
          bold: true,
          fontSize: 6,
          color: 'black'
        },
        tablecontent: {
          bold: true,
          fontSize: 6,
          color: 'black'
        }
      }
    };
    pdfMake.createPdf(dd).open();
  }
  openBoletaCalificaciones()
  {
    var headers = {
      fila_0: {
        col_0: {
          text: 'Nº',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          alignment: 'center',
          margin: [0, 30, 0, 0],
          bold: true
        },
        col_1: {
          text: 'ASIGNATURAS',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2 ,
          alignment: 'center',
          margin: [0, 30, 0, 0],
          bold: true
        },
        col_2: {
          text: 'PRIMERA \n EVALUACIÓN',
          style: 'tableHeader',
          colSpan: 2,
          alignment: 'center',
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_3: {
          text: '',
        },
        col_4: {
          text: 'SEGUNDA \n EVALUACIÓN',
          style: 'tableHeader',
          colSpan: 2,
          alignment: 'center',
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_5: {
          text: '',
        },
        col_6: {
          text: 'TERCERA \n EVALUACIÓN',
          style: 'tableHeader',
          colSpan: 2,
          alignment: 'center',
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_7: {
          text: '',
        },
        col_8: {
          text: 'CUARTA \n EVALUACIÓN',
          style: 'tableHeader',
          colSpan: 2,
          alignment: 'center',
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_9: {
          text: '',
        },
        col_10: {
          // text: 'PROM. \n TEÓRICO',
          // style: 'tableHeader',
          // colSpan: 1,
          // rowSpan:2,
          // alignment: 'center',
          // margin: [0, 15, 0, 0],
          // bold: true
          image: this.writeRotatedText('PROMEDIO','TEÓRICO',''), fit:[25,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_11: {
          // text: 'PROM. \n PRÁCTICO',
          // style: 'tableHeader',
          // colSpan: 1,
          // rowSpan:2,
          // alignment: 'center',
          // margin: [0, 15, 0, 0],
          // bold: true
          image: this.writeRotatedText('PROMEDIO','PRÁCTICO',''), fit:[25,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_12: {
          // text: 'PROM.\nFINAL',
          // style: 'tableHeader',
          // colSpan: 1,
          // rowSpan:2,
          // alignment: 'center',
          // margin: [0, 15, 0, 0],
          // bold: true
          image: this.writeRotatedText('PROMEDIO','FINAL',''), fit:[25,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_13: {
          // text: 'PRUEBA DE\n RECUPERACIÓN',
          // style: 'tableHeader',
          // colSpan: 1,
          // rowSpan:2,
          // alignment: 'center',
          // margin: [0, 15, 0, 0],
          // bold: true
          image: this.writeRotatedText('PRUEBA DE','RECUPERACIÓN',''), fit:[25,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:2,
          margin: [0, 4, 0, 0],
          bold: true
        },
        // col_14: {
        //   text: 'OBSERVACIÓN',
        //   style: 'tableHeader',
        //   colSpan: 1,
        //   rowSpan:2,
        //   alignment: 'center',
        //   margin: [0, 30, 0, 0],
        //   bold: true
        // },
      },

      fila_1: {
        col_0: {
          text: '',
        },
        col_1: {
          text: '',
        },
        col_2: {
          image: this.writeRotatedText2('EVALUACIÓN','TEÓRICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_3: {
          image: this.writeRotatedText2('EVALUACIÓN','PRÁCTICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_4: {
          image: this.writeRotatedText2('EVALUACIÓN','TEÓRICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_5: {
          image: this.writeRotatedText2('EVALUACIÓN','PRÁCTICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_6: {
          image: this.writeRotatedText2('EVALUACIÓN','TEÓRICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_7: {
          image: this.writeRotatedText2('EVALUACIÓN','PRÁCTICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_8: {
          image: this.writeRotatedText2('EVALUACIÓN','TEÓRICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_9: {
          image: this.writeRotatedText2('EVALUACIÓN','PRÁCTICA',''), fit:[15,60], alignment: 'center',
          style: 'tableHeader',
          colSpan: 1,
          margin: [0, 4, 0, 0],
          bold: true
        },
        col_10: {
          text: '',
        },
        col_11: {
          text: '',
        },
        col_12: {
          text: '',
        },
        col_13: {
          text: '',
        },
        // col_14: {
        //   text: '',
        // },
      }
    }


    var rows = this.cursos; //en Esta variable estan los datos

    var body = [];
    for (var key in headers) {
      if (headers.hasOwnProperty(key)) {
        var header = headers[key];
        var row = new Array();
        row.push(header.col_0);
        row.push(header.col_1);
        row.push(header.col_2);
        row.push(header.col_3);
        row.push(header.col_4);
        row.push(header.col_5);
        row.push(header.col_6);
        row.push(header.col_7);
        row.push(header.col_8);
        row.push(header.col_9);
        row.push(header.col_10);
        row.push(header.col_11);
        row.push(header.col_12);
        row.push(header.col_13);
        // row.push(header.col_14);

        body.push(row);
      }
    }
    //HASTA ACA TODO BIEN
    var contador = 0;
    for (var key in rows) {

      if (rows.hasOwnProperty(key)) {
        var data = rows[key];
        var row = new Array();
          contador = contador + 1;
          row.push({ text: contador, alignment: 'center',style: 'tablecontent'});
          // row.push( data.id.toString()  ); //0

          //HACIENDO ALGUNOS CALCULOS PARA COLOREAR
          var color1;
          var color2;
          var color3;
          var color4;
          var colorProm;
          var colorRecuperacion;
          (data.Teorica1+data.Practica1>60) ? color1='#000': color1 = '#ff5500';
          (data.Teorica2+data.Practica2>60) ? color2='#000': color2 = '#ff5500';
          (data.Teorica3+data.Practica3>60) ? color3='#000': color3 = '#ff5500';
          (data.Teorica4+data.Practica4>60) ? color4='#000': color4 = '#ff5500';
          (data.PromEvT+data.PromEvP>60) ? colorProm='#000': colorProm = '#ff5500';
          (data.PruebaRecuperacion>60) ? colorRecuperacion='#000': colorRecuperacion = '#ff5500';
          //PARA NOMBRE COMPLETO
          // var apPa = data.Ap_Paterno.toString();
          // var apMa = data.Ap_Materno.toString();
          // var nom = data.Nombre.toString();
          // var NomComplet = apPa + ' ' + apMa + ' ' + nom;

          row.push({ text: data.NombreCurso.toString(), alignment: 'center',style: 'tablecontent'}); //1
          (data.Teorica1==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.Teorica1.toString(), alignment: 'center',color:color1});//2
          (data.Practica1==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.Practica1.toString(), alignment: 'center',color:color1}); //3
          (data.Teorica2==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.Teorica2.toString(), alignment: 'center',color:color2});//4
          (data.Practica2==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.Practica2.toString(), alignment: 'center',color:color2}); //5
          (data.Teorica3==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.Teorica3.toString(), alignment: 'center',color:color3});//6
          (data.Practica3==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.Practica3.toString(), alignment: 'center',color:color3}); //7
          (data.Teorica4==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.Teorica4.toString(), alignment: 'center',color:color4});//8
          (data.Practica4==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.Practica4.toString(), alignment: 'center',color:color4}); //9
          (data.PromEvT==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.PromEvT.toString(), alignment: 'center',color:colorProm});//10
          (data.PromEvP==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.PromEvP.toString(), alignment: 'center',color:colorProm});//11
          (data.Promedio==null) ? row.push({ text: '', alignment: 'center'}) :row.push({ text: data.Promedio.toString(), alignment: 'center',color:colorProm});//12
          (data.PruebaRecuperacion==null) ? row.push({ text: '', alignment: 'center'}) : row.push({ text: data.PruebaRecuperacion.toString(), alignment: 'center',color:colorRecuperacion}); //13
          body.push(row);

      }
    }

    //PARA LA PARTE DE OBSERVACIONES HACER OTRO BODY
    var headersobs = {
      fila_0: {
        col_0: {
          text: 'OBSERVACIONES',
          style: 'tableHeader',
          colSpan: 3,
          rowSpan:1,
          alignment: 'center',
          margin: [0, 0, 0, 0],
          bold: true
        },
        col_1: {
        },
        col_2: {
        },
      },
      fila_1: {
        col_0: {
          text: 'Nº',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:1,
          alignment: 'center',
          margin: [0, 0, 0, 0],
          bold: true
        },
        col_1: {
          text: 'ASIGNATURAS',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:1,
          alignment: 'center',
          margin: [0, 0, 0, 0],
          bold: true
        },
        col_2: {
          text: 'OBSERVACION',
          style: 'tableHeader',
          colSpan: 1,
          rowSpan:1,
          alignment: 'center',
          margin: [0, 0, 0, 0],
          bold: true
        },
      },
    }
    var bodyobs = [];
    for (var key in headersobs) {
      if (headersobs.hasOwnProperty(key)) {
        var header = headersobs[key];
        var row = new Array();
        row.push(header.col_0);
        row.push(header.col_1);
        row.push(header.col_2);

        bodyobs.push(row);
      }
    }
    var contador = 0;
    for (var key in rows) {

      if (rows.hasOwnProperty(key)) {
        var data = rows[key];
        var row = new Array();
          contador = contador + 1;
          row.push({ text: contador, alignment: 'center',style: 'tablecontent'});
          // row.push( data.id.toString()  ); //0

          row.push({ text: data.NombreCurso.toString(), alignment: 'center',style: 'tablecontent'}); //1

          if (data.Primero.toString()!='0') {
            row.push({ text: data.Primero.toString(), alignment: 'center',style: 'tablecontent'}); //2
          } else {
            row.push({ text: 'NINGUNA OBSERVACIÓN', alignment: 'center',style: 'tablecontent'}); //2
          }


          bodyobs.push(row);

      }
    }


    // var paginaInicial = this.numeroPaginaCurso - 1;
    var dd = {
      //izq, arriba,derecha

      pageSize: 'LETTER',
      pageMargins: [ 40, 40, 40, 60 ],
      // pageOrientation: 'landscape',

      // footer: function (currentPage, pageCount) {
      //   return [{
      //       text: '' + (currentPage + paginaInicial).toString() + '',
      //       alignment: 'right',
      //       margin: [0, -700, 50, 0],fontSize: 9
      //     },

      //   ];
      // },
      defaultStyle: {
        fontSize: 8,
        columnGap: 20
      },
      content: [
        {text:'INSTITUTO DE FORMACIÓN ARTISTICA', fontSize:9, alignment: 'center', bold:true},
        {text:'" MARÍA LUISA LUZIO "', fontSize:9, alignment: 'center', bold:true},
        {text:' BOLETÍN DE CALIFICACIONES ', fontSize:9, alignment: 'center', bold:true},
        // {text:'\n'},
        {
          image: 'data:image/png;base64,',
          width: 60, height: 60,  alignment:'right',margin:[0,-45,0,0]
        },
        {
          columns: [
            {
              width: '*',
              text: this.NombreCompleto,
              bold: true
            },
            {
              width: '*',
              text: data.NivelCurso.toString(),
              bold: true
            },
            {
              width: '*',
              text: 'GESTIÓN 2022',
              bold: true
            },
          ]
        },
        {
          style: 'tableExample',
          table: {
            widths: ['auto', '*',25,25,25,25,25,25,25,25,20,20,20,20],
            // headerRows: 1,
            // keepWithHeaderRows: 1,
            body: body
          },

          layout: {
            hLineWidth: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 2 : 1;
            },
            vLineWidth: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 2 : 1;
            },
            hLineColor: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 'black' : 'gray';
            },
            vLineColor: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 'black' : 'gray';
            },
            // hLineStyle: function (i, node) { return {dash: { length: 10, space: 4 }}; },
            // vLineStyle: function (i, node) { return {dash: { length: 10, space: 4 }}; },
            // paddingLeft: function(i, node) { return 4; },
            // paddingRight: function(i, node) { return 4; },
            // paddingTop: function(i, node) { return 2; },
            // paddingBottom: function(i, node) { return 2; },
            // fillColor: function (rowIndex, node, columnIndex) { return null; }
          }
        },
        {
          text:'\n\n'
        },
        {
          style: 'tableExample',
          table: {
            widths: ['auto','auto','*'],
            body: bodyobs
          },

          layout: {
            hLineWidth: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 2 : 1;
            },
            vLineWidth: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 2 : 1;
            },
            hLineColor: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 'black' : 'gray';
            },
            vLineColor: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 'black' : 'gray';
            },
          }
        },
        {
          text:'\n\n'
        },
        {text:' 1. ESCALA DE VALORACIÓN', fontSize:9, bold:true},
        {text:'El rendimiento académico de las y los estudiantes será expresado en la escala de notas de 1 a 100 puntos, el rendimiento académico podrá expresarse en conceptos como:', fontSize:9, bold:false},
        {
          text:'\n'
        },
        {
          style: 'tableExample',
          table: {
            widths: [80, 80],
            body: [
              [{text:'Calificación', fontSize:9, bold:true},{text:'Valoración', fontSize:9, bold:true}],
              [{text:'1 a 60', fontSize:9, bold:false}, {text:'Reprobación', fontSize:9, bold:false}],
              [{text:'61 a 100', fontSize:9, bold:false}, {text:'Aprobación', fontSize:9, bold:false}]
            ]
          },

          layout: {
            hLineWidth: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 2 : 1;
            },
            vLineWidth: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 2 : 1;
            },
            hLineColor: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 'black' : 'gray';
            },
            vLineColor: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 'black' : 'gray';
            },
          }
        },
        {
          text:'\n'
        },
        {text:' 2. PONDERACIÓN', fontSize:9, bold:true},
        {text:'La calificación final de cada asignatura será distribuida de acuerdo al siguiente detalle:', fontSize:9, bold:false},
        {
          text:'\n'
        },
        {
          style: 'tableExample',
          table: {
            widths: [80, 80],
            body: [
              [{text:'70%', fontSize:9, bold:false}, {text:'Práctica', fontSize:9, bold:false}],
              [{text:'30%', fontSize:9, bold:false}, {text:'Teoría', fontSize:9, bold:false}]
            ]
          },

          layout: {
            hLineWidth: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 2 : 1;
            },
            vLineWidth: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 2 : 1;
            },
            hLineColor: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 'black' : 'gray';
            },
            vLineColor: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 'black' : 'gray';
            },
          }
        },
        {
          text:'\n\n\n\n\n\n'
        },
        {
          columns: [
            {
              alignment:'center',
              width: '*',
              text: '_____________________________ \nDIRECTOR ACADÉMICO',
              marginLeft: 5,
              bold: true
            },
            {
              alignment:'center',
              width: '*',
              text: '___________________________ \nPADRE DE FAMILIA',
              marginLeft: -20,
              bold: true
            },
          ]
        },
      ],

      styles: {
        headers: {
          fontSize: 28,
          bold: true
        },
        subheader: {
          fontSize: 15,
          bold: true
        },
        quote: {
          italics: true
        },
        small: {
          fontSize: 8
        },
        sta: {
          fontSize: 11,
          bold: false,
          alignment: 'justify'
        },
        tableHeader: {
          bold: true,
          fontSize: 6,
          color: 'black'
        },
        tablecontent: {
          bold: false,
          fontSize: 6,
          color: 'black'
        }
      }
    };
    pdfMake.createPdf(dd).open();
  }
}
